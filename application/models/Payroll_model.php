...
class Payroll_model extends MY_Model
{
    ...
    public function searchPayment($id, $month, $year)
    {
        ...
        $query = $this->db->select('staff.name,staff.surname,staff.employee_id,staff.basic_salary,staff_payslip.*')->where(array('staff_payslip.month' => $month, 'staff_payslip.year' => $year, 'staff_payslip.staff_id' => $id))->join("staff_payslip", "staff.id = staff_payslip.staff_id")
		->group_start()
			->where('staff.session_id', $this->current_session)
			->or_where('staff.session_id IS NULL', null, false)
		->group_end()->get("staff");
    ...
    public function getbetweenpayrollReport($start_date, $end_date)
    {
        ...
        $query     = $this->db->select('staff.id,staff.employee_id,staff.name,roles.name as user_type,staff.surname,staff_designation.designation,department.department_name as department,staff_payslip.*')->join("staff_payslip", "staff_payslip.staff_id = staff.id", "inner")->join("staff_designation", "staff.designation = staff_designation.id", "left")->join("department", "staff.department = department.id", "left")->join("staff_roles", "staff_roles.staff_id = staff.id", "left")->join("roles", "staff_roles.role_id = roles.id", "left")->where($condition)
		->group_start()
			->where('staff.session_id', $this->current_session)
			->or_where('staff.session_id IS NULL', null, false)
		->group_end()->get("staff");
