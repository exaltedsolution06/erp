...
class Batchsubject_model extends CI_Model {
    ...
    public function getExamSubjectsPrintReport($id,$term_id,$student_id) {

        $this->db->select('*')->from('exam_group_class_batch_exam_subjects');
		$this->db->where('exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id', $id);
        $this->db->where('exam_group_class_batch_exam_subjects.subject', $term_id);
        $this->db->where('exam_group_class_batch_exam_subjects.student_id', $student_id);
