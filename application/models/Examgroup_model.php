...
class Examgroup_model extends MY_Model {
	...
	public function get_c_by_exam_group($post_exam_group_id)
	{
		$this->db->select('
			coscholasticareas.*,
			exam_groups.name as exam_group_name,
		...
		// If single value → use normal WHERE
		} else {
			$this->db->where('coscholasticareas.exam_group', $post_exam_group_id);
		}
		$this->db->order_by('coscholasticareas.id');
		$query = $this->db->get();
		return $query->result();
    ...
    public function add_c($data) {
        ...
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('coscholasticareas', $data);
            $message = UPDATE_RECORD_CONSTANT . " On  exam groups id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            $this->db->insert('coscholasticareas', $data);
    ...
    public function add_exam_1($data) {
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('exam_group_class_batch_exams', $data);
            $message = UPDATE_RECORD_CONSTANT . " On  exam group exams name id " . $data['id'];
            ...
            // $exam_group = $this->examgroup_model->get_c($data['exam_group_id']);
            $this->db->insert('exam_group_class_batch_exams', $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On exam group exams name id " . $insert_id;
            $action = "Insert";
    ...
    public function getExamByExamGroup_1($id, $is_active = false) {
        $this->db->select('exam_group_class_batch_exams.*,sessions.session,(select COUNT(*) from exam_group_class_batch_exam_subjects WHERE exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id = exam_group_class_batch_exams.id) as `total_subjects`')->from('exam_group_class_batch_exams');
        $this->db->join('sessions', 'sessions.id = exam_group_class_batch_exams.session_id');
        if ($is_active) {
            $this->db->where('exam_group_class_batch_exams.is_active', $is_active);
    ...
    public function getExamByExamGroup_reportCard($id,$student_id, $is_active = false) {
        $this->db->select('exam_group_class_batch_exams.*,sessions.session,(select COUNT(*) from exam_group_class_batch_exam_subjects WHERE exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id = exam_group_class_batch_exams.id) as `total_subjects`')->from('exam_group_class_batch_exams');
    ...
    public function getExamByExamGroup_reportCard_c($id,$student_id, $is_active = false) {
        $this->db->select('exam_group_class_batch_exams.*, exam_group_exam_results_coscholastic.get_marks, exam_group_exam_results_coscholastic.attendence, sessions.session,(select COUNT(*) from exam_group_class_batch_exam_subjects WHERE exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id = exam_group_class_batch_exams.id) as `total_subjects`')->from('exam_group_class_batch_exams');
