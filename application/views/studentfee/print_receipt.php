					...
					<div class="col-sm-6" style="padding-left: 0px; padding-top:10px">
						...
						<?php } ?>
							...
							<table class="table mt-1 mb-1" style="border-top:1px solid #000000; border-bottom:1px solid #000000;">
								<thead>
									<tr>
										<th style="width:33%;">Rec. No.: <?=$fees[0]->receipt_no?></th>
										<th style="width:33%; text-align:center;"><?php echo isset($result->fee_receipt_first_copy) && $result->fee_receipt_first_copy != '' ? $result->fee_receipt_first_copy : 'School Copy'; ?></th>
										<th style="width:33%;" class="text-end">Date: <?=date('d-m-Y',strtotime($fees[0]->date_time))?></th>
									</tr>
								</thead>
							</table> 
								...
								<table class="table mt-1 mb-1" >
									...
									<?php  
										...
										{ 
											...
											{
												$fee_head_bal = format_amount($list->total);
											}else{
									...
									?>
										<tr>
											...
											<?php } else { ?>
												<td style="font-weight:bold" class="text-end"><?=format_amount($list->ledger_amt)?></td>
											<?php } ?>
										</tr>
									<?php 
											}
									...
									<?php if ($result->rcpt_total_amt) { ?>
										<tr style="border-top:1px solid">
											<td colspan="2" class="text-end" style="font-size:17px;"><strong>Total Amount</strong></td>
											<?php if($fees[0]->fee_head_name!='Ledger Amount'){ ?>
											<td class="text-end"><h6 style="font-size:17px;"><b><?=format_amount((int)$pay+(int)$fees[0]->ledger_amt+(int)$list->previous_balance+(int)$list->remaining_previous_balance+(int)$list->previous_discount)?></b></h6></td>
											<?php }else{ ?>
											<td class="text-end"><h6 style="font-size:17px;"><b><?=format_amount((int)$pay+(int)$list->previous_balance+(int)$list->remaining_previous_balance+(int)$list->previous_discount)?></b></h6></td>
											<?php } ?>
										</tr>
									<?php } if ($result->rcpt_late_fee) { ?>
										<tr>
											<td colspan="2" class="text-end">+ Late/Other Fee (If Any)</td>
											<td class="text-end"><?=format_amount($fees[0]->late_fees??0)?></td>
										</tr>
									<?php } if ($result->rcpt_discount_amt) { ?>
										<tr>
											<td colspan="2" class="text-end">- Discount Amount (If Any)</td>
											<td class="text-end"><?=format_amount((int)$fees[0]->discount_amt+(int)$list->previous_discount)?></td>
										</tr>
									<?php } if ($result->rcpt_net_amt) { ?>
										<tr style="border-top:1px solid">
											<td colspan="2" class="text-end" style="font-size:17px;"><strong>Net Fees</strong></td>
											<td class="text-end"><strong><h6 style="font-size:17px;"><b>
											<?php
											$ledger = $fees[0]->ledger_amt ?? 0;
											$late   = $fees[0]->late_fees ?? 0;
											$disc   = $fees[0]->discount_amt ?? 0;
											if($fees[0]->fee_head_name!='Ledger Amount'){ 
											  echo $total  = format_amount((int)$pay + (int)$ledger + (int)$late - (int)$disc + (int)$list->previous_balance + (int)$list->remaining_previous_balance);
											}else{
											  echo $total  = format_amount((int)$pay + (int)$late - (int)$disc + (int)$list->previous_balance + (int)$list->remaining_previous_balance);
											}
											?>
											</b></h6></strong></td>
										</tr>
									<?php } if ($result->rcpt_received_amt) { ?>
										<tr>
											<td colspan="2" class="text-end" style="font-weight:bold;font-size:17px;">Received Amount</td>
											<td class="text-end" style="font-weight:bold;font-size:17px;"><?=format_amount($fees[0]->receipt_amt)?></td>
										</tr>
									<?php } if ($result->rcpt_balance_amt) { ?>
										<tr>
											<td colspan="2" class="text-end" style="font-weight:bold;font-size:17px;">Balance Amount</td>
											<td class="text-end" style="font-weight:bold;font-size:17px;"><?=format_amount($fees[0]->balance_amt+$list->remaining_previous_balance)?></td>
										</tr>
									<?php } ?>
									</tbody>
								</table>
							...
							<div class="row">
								<div class="col-12"><h6><b>Received</b> : <?=number_to_words((int)$fees[0]->receipt_amt);?> Only</h6></div>
							</div>
							<?php } ?>
							<div class="row">
								<div class="col-8">
						...
						<?php } ?>
							<table class="table mt-1 mb-1">
								<thead>
									<tr style="border-top:1px solid #000000; border-bottom:1px solid #000000;">
										<th style="width:33%;">Rec. No.: <?=$fees[0]->receipt_no?></th>
										<th style="width:33%; text-align:center;"><?php echo isset($result->fee_receipt_second_copy) && $result->fee_receipt_second_copy != '' ? $result->fee_receipt_second_copy : 'Parent Copy'; ?></th>
										<th style="width:33%;" class="text-end">Date: <?=date('d-m-Y',strtotime($fees[0]->date_time))?></th>
									</tr>
								</thead>
							</table>
							...
							<div style="padding:1px">
								<table class="table mt-1 mb-1" >
									...
									<?php } } ?>
									<?php $pay=0; foreach($fees as $list){
											if($list->fee_head_name != 'Ledger Amount')
											{
												$fee_head_bal = format_amount($list->total);
											}else{
									...
									<?php } if ($result->rcpt_net_amt) { ?>
									  ...
									  <?php } if ($result->rcpt_balance_amt) { ?>
										<tr>
											<td colspan="2" class="text-end" style="font-weight:bold;font-size:17px;">Balance Amount</td>
											<td class="text-end" style="font-weight:bold;font-size:17px;"><?=format_amount((int)$fees[0]->balance_amt+$list->remaining_previous_balance)?></td>
										</tr>
									  <?php } ?>
									</tbody>
								</table>
            ...
            <div  style="border:1px solid;" >
				...
				<?php } ?>
                    ...
                    <table class="table mt-1 mb-1" >
                        ...
                        <tr>
                            <td colspan="2" class="text-end" style="font-weight:bold;font-size:17px;">Balance Amount</td>
                            <td class="text-end" style="font-weight:bold;font-size:17px;"><?=format_amount((int)$fees[0]->balance_amt+(int)$list->remaining_previous_balance)?></td>
                        </tr>
						<?php } ?>
                        </tbody>
                    </table>
