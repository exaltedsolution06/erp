...
<div class="modal fade" id="examModal">
    <div class="modal-dialog modal-lg">
        <form id="formadd" action="<?php echo site_url('admin/coscholasticareas/ajaxaddexam'); ?>">
...
?>
						...
						<?php 
                            if($this->rbac->hasPrivilege('co_publish','can_add')){
                                ?>
						...
						<?php 
                            } if($this->rbac->hasPrivilege('co_publish_result','can_add')){
                                ?>
                             <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
                            <div class="checkbox">
                                <label>
...
<div id="addSubject" class="modal fade" role="dialog">
    <div class="modal-dialog modal-xl">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">Add Terms Subject</h4>
            </div>
...
<div id="allotStudentModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-xl">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="allotStudentForm" action="<?php echo site_url('admin/coscholasticareas/examstudent') ?>" method="post" >
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#confirm-delete').on('click', '.btn-ok', function (e) {
            ...
            $.ajax({
                type: "post",
                url: '<?php echo site_url("admin/coscholasticareas/deleteExam") ?>',
                dataType: 'JSON',
                data: {'id': id},
                beforeSend: function () {
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.editexamModalButton', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: base_url + "admin/coscholasticareas/getExamByID",
        ...
        $(document).on('click', '#subjectModalButton', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: base_url + "admin/coscholasticareas/getexamSubjects",
                ...
                success: function (data) {
                    ...
                    batch_class = data.batch_class_dropdown;
                    if (data.exam_class_count > 0) {
                        x = data.exam_class_count + 1;
                    }
    ...
    $('#examconnectModal').on('show.bs.modal', function (e) {
        ...
        $.ajax({
            type: "POST",
            url: base_url + "admin/coscholasticareas/connectexams",
...
<script type="text/javascript">
    ...
    function all_records() {
        $.ajax({
            type: "POST",
            url: base_url + "admin/coscholasticareas/getexam",
...
<script>
    $(document).on('click', '.add', function () {
        ...
        html += '<td width="150"><select onchange="GetFilterData('+ x +')"  id="ClassData'+x+'" name="class_' + x + '" class="form-control item_unit tddm200">' + batch_class + '</select></td>';
        
       



        html += '<td width="150"><select onchange="GetFilterDataStudent('+ x +')" id="ClassSubject'+ x +'" name="session_' + x + '" class="form-control item_unit tddm200"><option value=""> Select Section </option> </select></td>';

        
        html += '<td width="150"><select id="StudentData'+x+'" name="student_' + x + '" class="form-control item_unit tddm200"><option>Select Student</option></select></td>';
       ...
       html += '<td class=""><input type="hidden" name="rows[]" value="' + x + '"> <input name="prev_row[' + x + ']" type="hidden" value="0"><input type="text" name="min_marks_' + x + '" class="form-control min_marks" /></td>';
    ...
    function GetFilterData(id){
        var class_id=$("#ClassData"+id).val();
        $.ajax({
            type: "get",
            url: "<?=base_url()?>/admin/coscholasticareas/getClass",
            data: {'classid':class_id}, 
            success: function (response)
            {
                $("#ClassSubject"+id).html(response);
            }
        });
    }


    function GetFilterDataStudent(id){
        var class_id=$("#ClassData"+id).val();
        var section_id=$("#ClassSubject"+id).val();
        
         $.ajax({
            type: "get",
            url: "<?=base_url()?>/admin/coscholasticareas/getStudent",
            data: {'classid':class_id,'section_id':section_id}, 
            success: function (response)
            {
                $("#StudentData"+id).html(response);
            }
        });
    }
...
<script type="text/javascript">
    ...
    $(document).on('click', '.examMarksSubject', function () {
        ...
        $.ajax({
            type: 'POST',
            url: baseurl + "admin/coscholasticareas/getSubjectByExam",
