<?php
	...
	// echo'<pre>';print_r($saved_json);exit;
?>
...
?>	
	...
	?>
		<img src="<?php echo base_url('uploads/marksheet/' . $desc->background_image); ?>" class="tcmybg" width="100%" height="100%" />
	<?php
		}
	?>
		<div class="row maincontent">
			...
			$is_header_file_path = FCPATH . 'uploads/marksheet/' . $desc->header_img;
						...
						<table class="w-100 table-bordered border-dark text-center" style="padding-bottom:0px">
							...
							<thead>
								...
								<tr>
									...
									$exam_type=$this->db->query("SELECT exam_group_class_batch_exams.* FROM exam_group_class_batch_exams INNER JOIN exam_group_class_batch_exam_students ON exam_group_class_batch_exam_students.exam_group_class_batch_exam_id=exam_group_class_batch_exams.id WHERE exam_group_id='".$rowDatagroup->id."' AND exam_group_class_batch_exams.id='".$post_exam_id."' AND exam_group_class_batch_exam_students.student_id='".$stddata->student_id."'")->result();
									// $allNames[] = $rowDatagroup->name;
									$gradecount = 0;
									if(isset($saved_json[0]) && $saved_json[0] == 1){
										$gradecount++;
									}
									if(isset($saved_marks_obtained_json[0]) && $saved_marks_obtained_json[0] == 1){
										$gradecount++;
									}
									if(isset($saved_max_marks_json[0]) && $saved_max_marks_json[0] == 1){
										$gradecount++;
									}
										?>
								...
								?>
										...
										<tr>
											...
											foreach($post_exam_group_id as $post_exam_group){
												...
												$exam_type=$this->db->query("SELECT exam_group_class_batch_exams.* FROM exam_group_class_batch_exams INNER JOIN exam_group_class_batch_exam_students ON exam_group_class_batch_exam_students.exam_group_class_batch_exam_id=exam_group_class_batch_exams.id WHERE exam_group_id='".$exam_group_id."' AND exam_group_class_batch_exams.id='".$post_exam_id."' AND exam_group_class_batch_exam_students.student_id='".$stddata->student_id."'")->result();
		...
		$resultData=$this->db->query("SELECT exam_group_exam_results.*,exam_group_class_batch_exam_subjects.max_marks FROM exam_group_exam_results left JOIN exam_group_class_batch_exam_subjects ON exam_group_class_batch_exam_subjects.id=exam_group_exam_results.exam_group_class_batch_exam_subject_id left JOIN exam_group_class_batch_exam_students ON exam_group_class_batch_exam_students.id=exam_group_exam_results.`exam_group_class_batch_exam_student_id` WHERE exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id='".$type->id."' and exam_group_class_batch_exam_students.exam_group_class_batch_exam_id='".$type->id."' and exam_group_class_batch_exam_subjects.subject_id='".$rowdata->id."' and exam_group_class_batch_exam_students.student_id='".$stddata->student_id."'")->result()[0];
													...
													$medical_leave = 2;
													$medical_leave_row = 2; //present
													}else{
														if($medical_leave != 2){
															$medical_leave = 1;														
														}
														if($medical_leave_row != 2){												
															$medical_leave_row = 1;	 //ml														
														}
													}
												
													$marks = !empty($resultData) ? ($resultData->attendence == 'absent' ? 'AB' : ($resultData->attendence == 'm_leave' ? 'ML' : $resultData->get_marks)) : "-";
													// echo $marks;
											...
											<?php
												...
												// $ml_array = [ 'type'=>'medical_leave' ];
												// array_push($array,$ml_array);
											?>
											</td>
											...
											if(isset($saved_json[0]) && $saved_json[0] == 1){
												$grade_array = [];
												$grade_array = [ 'type'=>'grade', 'marks'=>$max_marks1, 'is_leave'=>$medical_leave ];
												// array_push($array,$max_marks1);
											...
											//echo ($gd[0]->name);
												// echo ($gd[0]->name);
											if($medical_leave == 1 && $medical_leave_row == 1){
												echo '';
											}else{
												echo ($gd[0]->name);
											}
		...
		$resultData_op=$this->db->query("SELECT exam_group_exam_results.*,exam_group_class_batch_exam_subjects.max_marks FROM exam_group_exam_results left JOIN exam_group_class_batch_exam_subjects ON exam_group_class_batch_exam_subjects.id=exam_group_exam_results.exam_group_class_batch_exam_subject_id left JOIN exam_group_class_batch_exam_students ON exam_group_class_batch_exam_students.id=exam_group_exam_results.`exam_group_class_batch_exam_student_id` WHERE exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id='".$type_op->id."' and exam_group_class_batch_exam_students.exam_group_class_batch_exam_id='".$type_op->id."' and exam_group_class_batch_exam_subjects.subject_id='".$rowdata->id."' and exam_group_class_batch_exam_students.student_id='".$stddata->student_id."'")->result()[0];	
												...
												if($resultData_op->attendence != 'm_leave'){
													...
													$medical_leave_op = 2;
													$medical_leave_row_op = 2;
												}else{
													if($medical_leave_op != 2){
														$medical_leave_op = 1;															
													}
													if($medical_leave_row_op != 2){
														$medical_leave_row_op = 1;															
													}
												}
												?>
													
														<?php
															$marks_op = !empty($resultData_op) ? ($resultData_op->attendence == 'absent' ? 'AB' : ($resultData_op->attendence == 'm_leave' ? 'ML' : $resultData_op->get_marks)) : "-";
															// echo $marks_op;
											...
											//echo ($gd[0]->name);
												// echo ($gd_op[0]->name);
											if($medical_leave_op == 1 && $medical_leave_row_op == 1){
												echo '';
											}else{
												echo ($gd_op[0]->name);
											}
							...
							<?php
								...
								foreach ($array1 as $row) {
									foreach ($row as $index => $value) {
										...
										elseif (is_array($value) && isset($value['marks'])) {
											...
											// if any leave found
											if (isset($value['is_leave']) && $value['is_leave'] == 2) {
												$isLeave[$index] = 2;
											}else{
												if($isLeave[$index] != 2){
												$isLeave[$index] = 1;
										...
										// grade block
										/*elseif (is_array($value) && isset($value['is_leave'])) {
											if (!isset($isLeave[$index])) $isLeave[$index] = 0;
											$isLeave[$index] += $value['is_leave'];
										}*/
									}
								}
								...
								$final = [];
								// echo '<pre>'; print_r($gradeLimits);
								// echo '<pre>'; print_r($isLeave);
								$final_row_grade = 0;
								foreach ($array1[0] as $index => $colExample) {
									...
									elseif (is_array($colExample) && isset($colExample['marks'])) {
										...
										// if any row has leave
										if (isset($isLeave[$index]) && $isLeave[$index] == 1) {
											$final[] = '';
											if($final_row_grade != 2){
											$final_row_grade = 1;
											}
											continue;
										}else{
			...
			<?php if($final_row_grade == 1){ ?>
				<div class="col-4 mb-3 d-flex" style="justify-content:end;">
					<strong><?php echo $this->lang->line('overall'); ?> <?php echo $this->lang->line('percentage'); ?>(%) : </strong>
					<strong class="text-underline" style="width:100px;">&nbsp; </strong>%
				</div>
			<?php } else { ?>
				<div class="col-4 text-end" style="padding: 0;">
				...
				<div class="row text-center" style="background-color: transparent">
					<div class="col-4">
						...
						if($desc->is_class_teacher==1){
							...
							$is_left_sign_file_path = FCPATH . 'uploads/marksheet/' . $desc->left_sign;
								if (file_exists($is_left_sign_file_path)) {						
							?>
							<img src="<?php echo base_url('uploads/marksheet/'.$desc->left_sign) ?>" style="height:90px;width:auto;margin-top: 5px;">
							<?php
								}else{
					...
					<div class="col-4">
						...
						if($desc->is_examination_ic==1){
							...
							$is_middle_sign_file_path = FCPATH . 'uploads/marksheet/' . $desc->middle_sign;
								if (file_exists($is_middle_sign_file_path)) {						
							?>
							<img src="<?php echo base_url('uploads/marksheet/'.$desc->middle_sign) ?>" style="height:90px;width:auto;margin-top: 5px;">
							<?php
								}else{
									echo '<div style="height:90px;width:auto;margin-top: 5px;"></div>';
								}
					...
					<div class="col-4">
						...
						if($desc->is_principal==1){
							...
							$is_right_sign_file_path = FCPATH . 'uploads/marksheet/' . $desc->right_sign;
								if (file_exists($is_right_sign_file_path)) {						
							?>
							<img src="<?php echo base_url('uploads/marksheet/'.$desc->right_sign) ?>" style="height:90px;width:auto;margin-top: 5px;">
							<?php
								}else{
									echo '<div style="height:90px;width:auto;margin-top: 5px;"></div>';
								}
