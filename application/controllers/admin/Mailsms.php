...
class Mailsms extends Admin_Controller {
    ...
    public function send_whatsapp_msg() {
        if (!$this->rbac->hasPrivilege('send_whatsapp', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Communicate');
        $this->session->set_userdata('sub_menu', 'mailsms/send_whatsapp');
        $data['title'] = 'Send WhatsApp Message';
		$class = $this->class_model->get();
        $data['classlist'] = $class;
        $userdata = $this->customlib->getUserData();
        $carray = array();
        ...
        $data['sch_setting'] = $this->sch_setting_detail;
        $this->load->view('layout/header');
        $this->load->view('admin/mailsms/whatsapp_msg', $data);
        $this->load->view('layout/footer');
    }
