...
class Route extends Admin_Controller {
    ...
    public function index() {
        if (!$this->rbac->hasPrivilege('route', 'can_view')) {
            access_denied();
        }
    ...
    function create() {
        if (!$this->rbac->hasPrivilege('route', 'can_add')) {
            access_denied();
        }
    ...
    function edit($id) {
        if (!$this->rbac->hasPrivilege('route', 'can_edit')) {
            access_denied();
        }
        $data['title'] = 'Add Route';
        $data['id'] = $id;
    ...
    function delete($id) {
        if (!$this->rbac->hasPrivilege('route', 'can_delete')) {
            access_denied();
        }
	...
	*/
	...
	{
		$exists = $this->route_model->data_exists($data, $id);
