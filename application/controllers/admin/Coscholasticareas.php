...
class Coscholasticareas extends Admin_Controller {
...
//====================//
    ...
    public function addmark($id) {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/coscholasticareas/addmark', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function edit($id) {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/coscholasticareas/examgroupEdit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $is_active = 0;
    ...
    public function ajaxaddexam() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $inserted_id = $this->examgroup_model->add_exam_1($postarray);
    ...
    public function getexam() {
        ...
        $data['examList'] = $this->examgroup_model->getExamByExamGroup_1($examgroup_id);

        $data['exam_page'] = $this->load->view('admin/coscholasticareas/_partialexamList', $data, true);
    ...
    public function connectexams() {
        ...
        $data['exam_page'] = $this->load->view('admin/coscholasticareas/_partialexamListConnection', $data, true);
        echo json_encode($data);
    }
    ...
    public function getexamSubjects() {
        ...
        $data['examgroupDetail'] = $this->examgroup_model->getExamByID_1($exam_id);
        $data['exam_subjects'] = $this->batchsubject_model->getExamSubjects($exam_id);

    $data['batch_subjects'] = $this->db->query("SELECT * FROM exam_groups")->result_array();
    $data['batch_class'] = $this->db->query("SELECT * FROM classes")->result_array();

        $data['exam_id'] = $exam_id;
        $data['exam_subjects_count'] = count($data['exam_subjects']);
        $data['exam_class_count'] = count($data['batch_class']);



        $data['batch_subject_dropdown'] = $this->load->view('admin/coscholasticareas/_partialexamSubjectDropdown', $data, true);


        $data['batch_class_dropdown'] = $this->load->view('admin/coscholasticareas/_partialexamClassDropdown', $data, true);
        ...
        $data['subject_page'] = $this->load->view('admin/coscholasticareas/_partialexamSubjects', $data, true);
    ...
    public function getClass()
    {
        ...
        foreach($section as $list){
            echo '<option value="'.$list['section_id'].'">'.$list['section'].'</option>';
        }
    }
    ...
    public function getStudent(){
        ...
        $student_data=$this->student_model->getStudentByClassSectionID($classid,$section_id);
        // echo json_encode($student_data[0]);
        echo '<option value="">Select Student</option>';
        foreach($student_data as $list){
            echo '<option value="'.$list['id'].'">'.$list['firstname'].'</option>';
        }   
    }
    ...
    public function getSubjectByExam() {
        ...
        $data['examgroupDetail'] = $this->examgroup_model->getExamByID_1($id);
    ...
    public function addexamsubject() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($rows as $row_key => $row_value) {
                ...
                if ($update_id == 0) {
                    ...
                    $insert_array[] = array(
                            ...
                            'max_marks' => '1',
                            'min_marks' => '',
                            'grade' => @$this->input->post('min_marks_' . $row_value),
                            'class_id' => @$this->input->post('class_' . $row_value),
                            'section_id' => @$this->input->post('session_' . $row_value),
                            'student_id' => @$this->input->post('student_' . $row_value),
                            'subject' => @$this->input->post('subject_' . $row_value),
                        );
            ...
            $a=$this->examsubject_model->add($insert_array, $update_array, $not_be_del, $this->input->post('exam_group_class_batch_exam_id'));
            $array = array('status' => '1', 'error' => '', 'message' => $this->lang->line('success_message'));
        }
