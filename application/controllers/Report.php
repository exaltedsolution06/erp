...
class Report extends Admin_Controller
{
    ...
    public function onlinefees_report()
    {
        ...
        if($id != 0){
            ...
            $config['base_url'] = base_url('report/onlinefees_report');
            $config['total_rows'] = $total_rows;
            $config['per_page'] = $per_page;
            $config['uri_segment'] = 3;
            $config['reuse_query_string'] = TRUE; // keeps per_page in URL
        ...
        if(!empty($_POST['feesCard'])){
            ...
            //     foreach ($monthsPost as $m) {
            //         $m_escaped = $this->db->escape_str($m); // Prevent injection or breaking SQL
            //         $this->db->or_where("JSON_CONTAINS(route_head.months, '\"$m_escaped\"')", null, false);
            //     }
            //     $this->db->group_end(); // End OR group
            // }
            $query = $this->db->get();
    ...
    public function expense()
    {
        ...
        */
        ...
        // paginate
        $config['base_url'] = base_url('reports/expense');
        $config['total_rows'] = $this->Receipt_model->get_receipt_expense_count($filters);
        ...
        $data['receipt_data'] = $this->Receipt_model->get_receipt_expense($config['per_page'], $page,$filters);
    ...
    public function payroll()
    {
        ...
        */
        ...
        // paginate
        $config['base_url'] = base_url('reports/payroll');
        $config['total_rows'] = $this->Receipt_model->get_receipt_expense_count($filters);
        ...
        $data['receipt_data'] = $this->Receipt_model->get_receipt_expense_list($config['per_page'], $page,$filters);
    ...
    public function incomegroup()
    {
        ...
        // paginate
        $config['base_url'] = base_url('report/incomegroup');
        $config['total_rows'] = $this->Receipt_model->get_receipt_count_student($selectedClasses,$selectedFeeCat,$selectedroutes);
    ...
    public function expensegroup()
    {
        ...
        // // paginate
        // $config['base_url'] = base_url('report/expensegroup');
        ...
        // $data['receipt_data'] = $this->Receipt_model->get_receipt($config['per_page'], $page);
    ...
    public function search_api() {
        ...
        foreach ($resultlist as $student) {
            ...
            // You can customize these keys based on your data structure
            $results[] = [
                'id'             => $student->student_session_id,
                'name'           => $student->firstname . ' ' . $student->middlename . ' ' . $student->lastname,
                'admission_no'   => $student->admission_no ?? '',
                'from_date'      => '2025-04-01',  // You can pull real dates if available
                'to_date'        => '2025-04-17',
                'father'         => $student->father_name ?? '',
                'class'          => $student->class . '-' . $student->section,
                'ledger_balance' => $student->ledger_balance ?? '0.00'
            ];
        }

        // If no results, return a default "not found" entry
        if (count($results) == 0) {
            $results[] = [
                'name' => 'Data not found',
                'admission_no' => '',
