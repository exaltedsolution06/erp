...
class Studentfee extends Admin_Controller
{
    ...
    public function saveFee(){
        ...
        if(!empty($data['pay'][0])){
            ...
            $this->Receipt_model->update_student($data['student_id'],($data['balance_amt']-$data['prev_row_balance']),$data['prev_row_balance']);
           ...
           $id=$this->Receipt_model->insert_receipt($insert_data);
			...
			$pre_session = $this->session_model->getPreSession($this->current_session);
            $this->Receipt_model->update_student($data['student_id'],($data['balance_amt']-$data['prev_row_balance']),(int)($data['prev_row_balance']));
	...
	public function editFee(){
        ...
        if(!empty($data['pay'][0])){            
			...
			$balance_sheet = array(
				...
				'student_id'   => $data['student_id'],
				'receipt_no'   => $data['receipt_no'],
				'amount'   => $data['receipt_amt']+$data['previous_session_balance'],
				'date'   => date('Y-m-d H:i:s'),
			);
