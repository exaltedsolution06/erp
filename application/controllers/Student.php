...
class Student extends Admin_Controller
{
	...
	public function dobcertificate($id)
    {
        ...
        // var_dump($data['student']);
        // $this->load->view('layout/header', $data);
        $this->load->view('student/dob-certificate', $data);
        // $this->load->view('layout/footer', $data);
    }
       ...
       // $this->form_validation->set_rules('cast_category', $this->lang->line('cast_category'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data_insert = array(
                ...
                'father_other_no'    => $this->input->post('father_other_no'),
                'father_id_no'    => $this->input->post('father_id_no'),
                'mother_pan_no'    => $this->input->post('mother_pan_no'),
                'mother_aadhar_no'    => $this->input->post('mother_aadhar_no'),
                'mother_other_no'    => $this->input->post('mother_other_no'),
                'mother_id_no'    => $this->input->post('mother_id_no'),
            );
            ...
            $data_insert['cast_category']=$this->input->post('cast_category');
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                           ...
                           $insert_id = $this->student_model->add($insert_student_data[$i], $data_setting);
							...
							$existing_student = $this->db
								->where('admission_no', $adm_no)
								->get('students')
								->row();
							if ($existing_student) {
								// UPDATE
								$insert_student_data[$i]['id'] = $existing_student->id;

								$this->student_model->add(
									$insert_student_data[$i],
									$data_setting
								);

								$insert_id = $existing_student->id;

							} else {

								// INSERT
								$insert_id = $this->student_model->add(
									$insert_student_data[$i],
									$data_setting
								);
								if (!empty($insert_id)) {
    ...
    public function edit($id)
    {
       ...
       // $data['vehroutelist']       = $vehroute_result;
        ...
        if ($this->form_validation->run() == false) {
            ...
            $custom_field_post = $this->input->post("custom_fields[students]");
            // echo json_encode($custom_field_post);
            // die;
            ...
            $data['cast_category']=$this->input->post('cast_category');
