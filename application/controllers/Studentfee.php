...
class Studentfee extends Admin_Controller
{
    ...
    public function saveFee(){
        ...
        if(!empty($data['pay'][0])){
			...
			// echo $previous_session_balance."<br>";die;
			// echo $total_pay."<br>";
			// echo "<pre>";print_r($data);die;
            $paid=$data['pay'];
			...
			// die;
            // $data['balance_amt']
			if(!isset($data['row_selector'][2]) && isset($data['row_selector'][1])){
				$fees_discount = (int)$previous_ledger_balance + ((int)$data['balance_amt']-(int)$data['prev_row_balance']);
				$previous_session_balance = (int)$data['prev_row_balance'];
			}else if(!isset($data['row_selector'][2]) && !isset($data['row_selector'][1])){
				$fees_discount = (int)$previous_ledger_balance + ((int)$data['balance_amt']-(int)$data['prev_row_balance']);
			...
			$total_fees_discount = (int)$data['balance_amt']-(int)$data['prev_row_balance']-(int)$data['ledg_rec_discount']-(int)$data['ledg_row_balance'];
            $this->Receipt_model->update_student($data['student_id'],$fees_discount,$previous_session_balance,$total_fees_discount);
            $this->studentfeemaster_model->update_student_fees_master_c($data['previous_student_session_id'],$data['prev_rec_discount'],$pre_session->id,$this->balance_group);
			...
			$this->Receipt_model->insert_previous_balance($insert_data);
           ...
           $id=$this->Receipt_model->insert_receipt($insert_data);
			...
			$total_fees_discount = (int)$data['balance_amt']-(int)$data['prev_row_balance']-(int)$data['ledg_rec_discount']-(int)$data['ledg_row_balance'];
            $this->Receipt_model->update_student($data['student_id'],$fees_discount,$previous_session_balance,$total_fees_discount);
            $this->studentfeemaster_model->update_student_fees_master_c($data['previous_student_session_id'],(int)($data['prev_rec_discount']),$pre_session->id,$this->balance_group);
	...
	public function editFee(){
        ...
        if(!empty($data['pay'][0])){            
            ...
            foreach($paid as $key=>$value){
                if($value=='paid'){
                    foreach ($data['months'] as $keys => $month) {
						...
						/*if(isset($data['row_selector'][1]) && $data['row_selector'][1]=='on'){
							$data['total'][0] = $data['prev_rec_discount'] = $data['prev_rec_amount'] = $data['prev_row_balance'] = $data['old_prev_row_balance'] = $data['current_prev_balance'] = $data['prev_discount'] = 0;
						}*/
                        ...
                        $insert_data = array(
                            ...
                            'fees_received' => $data['hid_fees_received'],
                            'late_fees'    => $data['late_fees'],
                            // 'ledger_amt'   => (int)$data['old_ledger_amt'] - (int)(array_sum($data['row_balance'])),
                            'ledger_amt'   => (int)$data['old_ledger_amt'],
                            'total_fees'   => (float) ($data['hid_fees_received'] ?? 0) + (float) ($data['late_fees'] ?? 0) + (float) ($data['old_ledger_amt'] ?? 0),
                            'discount_amt' => (float) ($data['discount_amt'] ?? 0)-(float) ($data['prev_rec_discount'] ?? 0),
                            'net_fees'     => (float) ($data['net_fees'] ?? 0)-(float) ($data['prev_rec_amount'] ?? 0),
                            'receipt_amt'  => $data['receipt_amt'],
			...
			$up_disc = (int)$data['ledg_rec_discount'] - (int)$data['original_discount_ledger'];
			$total_fees_discount = ((int)$data['balance_amt']-(int)$data['prev_row_balance']-(int)$data['original_ledg_bal']-((int)$data['ledg_row_balance']-(int)$data['old_ledg_row_balance'])) - (int)$up_disc;
				
			if($data['hidden_received_discount_ledger'] > 0 && !isset($data['row_selector'][2])){
				// $up_ledg_bal =  ((int)$data['hidden_received_discount_ledger'] + (int)$data['prev_balance_amt_remain']) - (int)(array_sum($data['row_balance']));
				$up_ledg_bal =  ((int)$data['hidden_received_discount_ledger'] + (int)$data['prev_balance_amt_remain']);
				// hidden_received_discount_ledger = LEDG AMT(Discount+Received)
				// prev_balance_amt_remain = Existing LEDG AMT
				...
				$student_data = $this->student_model->get($data['student_id']);
				$ledger_received = $this->Receipt_model->ledger_received_sum($data['student_id']);
				$up_ledg_bal = ((int)$student_data['total_fees_discount']+(int)$total_fees_discount) - (int)$ledger_received->ledger_received;
					
				// $up_ledg_bal = (int)$data['balance_amt'] - (int)$data['prev_row_balance'] - (int)$data['original_ledg_bal'];
				// echo 'Two'.'<br>';
				// echo $data['balance_amt'].'<br>';
				// echo $data['prev_row_balance'].'<br>';
				// echo $up_ledg_bal.'<br>';die;
			}else{
				$cur_ledger_update = ((int)$data['balance_amt']-(int)$data['prev_row_balance']) - (int)$data['original_ledg_bal']; // (200-0)-200
				/*if(isset($data['row_selector'][2])) {
					$cur_ledger_update = (int)$data['ledg_rec_discount'] + (int)$data['ledg_rec_amount'];
				}*/
				$up_ledg_bal = (int)$data['prev_balance_amt_remain'] + $cur_ledger_update; // 4000+4000
				// original_ledg_bal = This receipt total ledger balance
				// prev_balance_amt_remain = Existing LEDG AMT
			...
			$this->Receipt_model->update_student($data['student_id'],$up_ledg_bal,$up_prev_bal,$total_fees_discount);
    ...
    public function deleteFee()
    {
        ...
        if (!$this->rbac->hasPrivilege('collect_fee', 'can_delete')) {
            access_denied();
        }
    ...
    public function edit($receipt_no)
    {
		...
		if(!empty($_POST['months']) || !empty($unique_months)){
			...
			$data['previous_balance'] = $receiptArr[0]['previous_balance'];
			$data['remaining_previous_balance'] = $receiptArr[0]['remaining_previous_balance'];
			$data['previous_discount'] = $receiptArr[0]['previous_discount'];
			...
			$data['ledger_discount'] = $receiptArr[0]['ledger_discount'];
			$data['previous_discount'] = $receiptArr[0]['previous_discount'];
        ...
        /*if (!$this->rbac->hasPrivilege('collect_fee', 'can_edit')) {
            access_denied();
        }
    ...
    public function studentfeelist(){
        ...
        if ($_GET['type'] == 'delete' && !empty($_GET['receipt_no'])) {
			...
			$res_del=$this->db
                ->select('student_id,session_id,date_time,back_id,receipt_no,mode,fee_head,late_fees,ledger_amt,total_fees,discount_amt,net_fees,receipt_amt,balance_amt,remarks,fee_head_name,SUM(balance_amount) as balance_amount,(total) as total, SUM(rec_discount) as rec_discount, SUM(rec_amount) as rec_amount, previous_balance, previous_discount, ledger_discount, remain_ledger, remaining_previous_balance')
                ->where_in('receipt_no', $receipt_no)
                ->group_by('fee_head')
                ->get('receipts')
                ->row();
            ...
            //if($res_del->fee_head_name=='Ledger Amount'){
                ...
                if ($query->num_rows() > 0) {
                    $row = $query->row();
                    $current_discount = (int)$row->fees_discount;
                    $current_previous_balance = (int)$row->previous_session_balance;
        ...
        // paginate
        $per_page_input = $this->input->get('per_page');
        $total_rows = $this->Receipt_model->get_receipt_count($from_date, $to_date, $mode, $studentId);
        ...
        $data['receipt_data'] = $this->Receipt_model->get_receipt($config['per_page'], $page,$from_date, $to_date, $mode, $studentId);
        $data['pagination_links'] = $this->pagination->create_links();
...
// studentfee_deletedlist
    ...
    public function studentfee_deletedlist(){
		...
		if ($_GET['type'] == 'delete' && !empty($_GET['receipt_no'])) {
			...
			// update table student_session table 
				...
				if ($query->num_rows() > 0) {
					...
					/*if(($ledger_amt + $balance_amount) > $current_discount){
						$this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">This receipt ledger balance is greater than the current ledger balance. Please permanently delete this receipt and create a new receipt again.</div>');
		...
		if ($_GET['type'] == 'permanent_delete' && !empty($_GET['receipt_no'])) {
			...
			$this->Receipt_model->delete_previous_balance($delete_prev);
